...
const quest = { id: "quest_research", slug: "open-research", title: "Open Research Quest", goal: "Build a source-backed map of an open research question.", description: "", status: "active" as const, created_at: timestamp, updated_at: timestamp, organization };
...
describe("OpenQuest live-domain contracts", () => {
  ...
  it("uses one compact work stream with pending, open, and accepted presentation", () => {
    ...
    const review = { stream_state: "review" as const, quest: { id: quest.id, slug: quest.slug, title: quest.title, organization }, challenge: streamChallenge, contribution: { id: "contribution_pending", actor_label: "Contributor A", summary: "Pending public work", status: "pending" as const, created_at: timestamp } };
    const open = { stream_state: "open" as const, quest: { id: quest.id, slug: quest.slug, title: quest.title, organization: null }, challenge: { ...streamChallenge, id: "challenge_2", status: "open" as const }, contribution: null };
    const resolved = { stream_state: "resolved" as const, quest: { id: quest.id, slug: quest.slug, title: quest.title, organization }, challenge: { ...streamChallenge, id: "challenge_3", status: "resolved" as const }, contribution: { id: "contribution_accepted", actor_label: "Contributor B", summary: "Accepted public work", status: "accepted" as const, created_at: timestamp } };
  ...
  it("publishes truthful contributor and freshness totals", () => {
    const observation = ObserveResponseSchema.parse({
      ...
      recent_contributors: [{ actor_label: "Contributor A", quest: { id: quest.id, slug: quest.slug, title: quest.title, organization }, last_event: "contribution.created", last_entity_id: "contribution_pending", last_summary: "Contribution submitted: Cross-check one claim", last_active_at: timestamp, activity_count: 2 }],
